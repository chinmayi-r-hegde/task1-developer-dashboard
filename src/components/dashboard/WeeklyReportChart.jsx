...
function getLast7Days() {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    days.push({ label: d.toLocaleDateString("en-IN", { weekday: "short" }), dateKey: d.toISOString().slice(0, 10) });
  }
  return days;
}
      ...
      {!hasData ? (
        ...
        <ResponsiveContainer width="100%" height={180}>
          <BarChart data={data}>
            ...
            <Tooltip contentStyle={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 8, fontSize: 12 }} cursor={{ fill: "var(--accent)", opacity: 0.08 }} />
