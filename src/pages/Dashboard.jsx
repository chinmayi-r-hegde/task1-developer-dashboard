...
export function Dashboard({ searchValue, activeView = "dashboard" }) {
  ...
  const [taskViewMode, setTaskViewMode] = useState("grid"); // "grid" | "kanban"
  ...
  const [confirmTarget, setConfirmTarget] = useState(null); // { type: "project"|"task", item }
  ...
  const toggleComplete = async (task) => {
    ...
    try {
      ...
      // mock/local-only task — optimistic update already applied, safe to ignore
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    if (type === "project") {
      ...
      try { await apiDeleteProject(item.id); } catch { /* ignore for mock/local */ }
    } else {
      setTasks((prev) => prev.filter((t) => t.id !== item.id));
      setSelectedTask(null);
      showToast(`Task "${item.title}" deleted`);
      try { await apiDeleteTask(item.id); } catch { /* ignore for mock/local */ }
    }
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <div className="flex border border-border rounded-lg overflow-hidden">
                <button
                  ...
                  className={`p-2 ${taskViewMode === "grid" ? "bg-accent text-white" : "text-secondary hover:text-primary"}`}
                  title="Grid view"
                >
                  <LayoutGrid size={14} />
                </button>
                <button
                  ...
                  className={`p-2 ${taskViewMode === "kanban" ? "bg-accent text-white" : "text-secondary hover:text-primary"}`}
                  title="Board view"
                >
                  <Columns3 size={14} />
                </button>
